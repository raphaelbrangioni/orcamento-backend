...
public interface CompraRepository extends JpaRepository<Compra, Long> {
    ...
    @Query("SELECT c FROM Compra c " +
            "WHERE (:descricao IS NULL OR LOWER(c.descricao) LIKE LOWER(CONCAT('%', :descricao, '%'))) " +
            "AND (:cartaoId IS NULL OR c.cartaoCredito.id = :cartaoId) " +
            "AND (:tipoDespesaId IS NULL OR c.tipoDespesa.id = :tipoDespesaId)")
